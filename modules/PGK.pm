...
require Exporter;
@ISA = qw(Exporter);
@EXPORT_OK = qw( ColorRow FontRow VBox HBox Table applyFont getGUI convertColor labelBox sayBox Pdie Pwait Pager Pfresh labeledRow labeledCol );
...
=head3 Usage
 ...
 my $graph = GRadar->new(spokes => 6)->prepare();
...
sub prepare {
	...
	return $self; # allows GRadar->new()->prepare() call
}
...
sub labelBox {
	...
	$box->insert( Label => text => "$label", autoHeight => 1, valignment => ta::Middle, alignment => ta::Left, pack => { fill => (defined $args{labfill} ? $args{labfill} : 'x'), expand => (defined $args{labex} ? $args{labex} : 0), }, $args{labelargs}->%* );
	return $box;
}
print ".";
...
sub labeledCol {
	my ($parent,$label,%args) = @_;
	return labeledRowCol($parent,$label,'V',%args);
}
print ".";
